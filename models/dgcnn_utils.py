...
def view_grouping(nsample, bin_map, xyz):
    device = bin_map.device
    B, N, C = bin_map.shape
    group_idx = torch.arange(N, dtype=torch.long).to(device).view(1, 1, N).repeat([B, N, 1])

    sqrdists1 = square_distance(bin_map, bin_map)
    sqrdists2 = square_distance(xyz, xyz)
...
def get_graph_feature_1(x, feature, k=20, idx=None):
    ...
    new_points = torch.cat([grouped_xyz_norm, x, torch.norm(grouped_xyz_norm, dim=-1, p=2).unsqueeze(-1),
     grouped_depth_norm], dim=-1).permute(0, 3, 1, 2).contiguous()
    
    # new_points = torch.cat((grouped_xyz-x, x), dim=3).permute(0, 3, 1, 2).contiguous()
    # new_points = torch.cat((grouped_feature-feature, feature), dim=3).permute(0, 3, 1, 2).contiguous()
    return new_points
