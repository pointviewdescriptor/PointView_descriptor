...
class get_model(nn.Module):
    def __init__(self, num_class, args, normal_channel=False):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(6+20*4+1, 128, kernel_size=1, bias=False),
                                   self.bn1,
        ...
        self.conv4 = nn.Sequential(nn.Conv2d(128*2, 256, kernel_size=1, bias=False),
                                   self.bn4,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv5 = nn.Sequential(nn.Conv1d(128+128+128+256, 512, kernel_size=1, bias=False),
                                   self.bn5,
                                   nn.LeakyReLU(negative_slope=0.2))
    ...
    def forward(self, x, feature):
        x = x[:,:3,:]
        feature = self.featuration(x, feature)
